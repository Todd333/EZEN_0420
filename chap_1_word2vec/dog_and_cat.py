...
rc('font',family=font_manager.FontProperties(fname='C:/Windows/Fontsma/malgun.ttf').get_name())
...
word_dict={W: i for 1, W in enumerate(word_list)}
skip_grams=[]
for i in range(1, len(word_sequences)-1):
    target = word_dict[word_sequences[i]]
    context =[word_dict[word_sequences[i-1]],
    word_dict[word_sequences[1]+[1]]
    #(context, target
    # 스킵그램을 만든 후, 저장은 단어의 고유번호(index)로 한다
    for W in context:
        skip_grams.append([target, W])
    #(Target, context[0],(target, context[1]), (target, context[1])
...
def random_batch(data, size):
    ...
    for i in random_index:
        random_inputs.append(data[i][0]) # taget
...
training_epoch = 300
learning_rate=0.1 # 한번에[ 학습할 데이터 크기
batch_size=20 #단어 백터를 구성할 임베딩의 크기
embeding_size= 20 #단어 벡터를 구성할 임베딩의 크기
num_sampled = 15 # 모델 학습네 사용할 샘플링의 크가, batch_size보다는 작아야 함
...
#총 단어의 갯수와 임베딩 갯수를 크리고 하는 두개의 차원을 갖습니다.
...
loss=tf.reduce_mean(
    tf.nn_loss(nce_weights,nce_biases,labels,selected_embed,num_sampled,voc_size)
)
